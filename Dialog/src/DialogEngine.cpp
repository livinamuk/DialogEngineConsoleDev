...
void DialogEngine::LoadQuestFile(std::string filename)
{
	...
	while (getline(file, line))
	{
		...
		// NPC?
			...
			//GetNPCPointer(npcName); // THis searches for the NPC, if it doesn't exist it creates it
		}
		else if (Util::StringContains(line, "DIALOG_ID_INACTIVE"))
			GetNPCPointer(npcName)->m_questDialogIDs.inactive = Util::StringToInt(Util::GetSubstringAfterColon(line));
		else if (Util::StringContains(line, "DIALOG_ID_ACTIVE"))
			GetNPCPointer(npcName)->m_questDialogIDs.active = Util::StringToInt(Util::GetSubstringAfterColon(line));
		else if (Util::StringContains(line, "DIALOG_ID_COMPLETED"))
			GetNPCPointer(npcName)->m_questDialogIDs.complete = Util::StringToInt(Util::GetSubstringAfterColon(line));
		else if (Util::StringContains(line, "REQUIREMENT_ITEM"))
			GetNPCPointer(npcName)->m_questDialogIDs.failed = Util::StringToInt(Util::GetSubstringAfterColon(line));
...
bool DialogEngine::EvaluateConditionalComparison(std::string name, ConditionType conditionType, float comparisonValue)
{
	...
	if (HasItem(name))
		objectValue = (float)GetItemQuantity(name);
...
void DialogEngine::SelectResponse(int index)
{
	...
	for (ResponseAction& responseAction : selectedRespone.m_responseActions)
	{
		...
		if (responseAction.m_actionType == ActionType::GIVE_ITEM) {
			DialogEngine::GiveItem(responseAction.m_name, (int)responseAction.m_modifierValue);
		}
		if (responseAction.m_actionType == ActionType::TAKE_ITEM) {
			DialogEngine::TakeItem(responseAction.m_name, (int)responseAction.m_modifierValue);
		}
	}
...
{
	for (size_t i = 0; i < s_gameItems.size(); i++) {
		// Find the item
...
{
	if (index < 0 || index >= (int)s_gameItems.size())
		return "out_of_range";
